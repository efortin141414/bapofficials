(function(){
 ...
 document.addEventListener('DOMContentLoaded',async()=>{
  ...
  try{
   ...
   const {data,error}=await db.from('members').select('member_id,first_name,middle_name,last_name,position,accreditation_level,photo_url,valid_until,active,public_profile,regions(name),chapters(name)').eq('member_id',id).eq('public_profile',true).maybeSingle();
